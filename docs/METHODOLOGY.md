...
Self-report alone is unreliable. Agents, like humans, overrate themselves. The 8D system combines multiple data sources into a composite score that corrects for self-assessment bias.

The scoring model blends objective telemetry, peer assessment, and self-assessment. Each source plays a distinct role: hard data captures what the agent does, peer input captures how others experience it, and self-assessment captures internal state the system cannot directly observe.

**TWC computation:** Total Wellness Coherence is a composite score derived from all 8 dimension scores. The framework accounts for how dimensions interact with each other, not just their individual values. See the [premium tier](https://github.com/divinityscience/8D360AI-premium) for the full scoring formula, coupling coefficients, and cascade modeling.

**Temporal smoothing:** Scores use temporal decay so recent assessments weigh more than older ones. This prevents stale assessments from masking current degradation.

**Three-source scoring** and the full mathematical model (including dimensional weighting, coupling coefficients, and cascade detection) are available in the [8D360AI-premium](https://github.com/divinityscience/8D360AI-premium) repository.
...
**Step 4: Compute TWC.** Repeat for all 8 dimensions, then apply the composite formula. See premium tier for advanced scoring.
...
| Metric | Definition |
|--------|-----------|
| TWC | Total Wellness Composite: composite score across all 8 dimensions. See premium tier for advanced scoring. |
