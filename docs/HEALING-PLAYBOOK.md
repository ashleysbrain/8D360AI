...
|------|---------|----------|---------------|
| **Health Observer Agent — Self-Heal** | Dimension < 7.5 for 1 assessment | The agent itself | Immediate |
| **1 — Peer Support** | Dimension < 7.Health Observer Agent for 2 consecutive assessments | Assigned peer | Within 24 hours |
| **2 — Agent-PA Review** | Dimension < 6.Health Observer Agent, or TWC declining 3+ weeks | Agent-PA | Within 4 hours |
| **3 — Ashley Escalation** | Dimension < 5.Health Observer Agent, burnout risk > Health Observer Agent.7Health Observer Agent, or novel failure | Ashley | Immediately |
...
| Circular reasoning or repetitive outputs | Output similarity scoring across tasks | Yellow when same-task output similarity > Health Observer Agent.85 |
...
| Freezing on edge cases | Timeout rate on novel inputs | Yellow at > 2Health Observer Agent% novel-input failures |
| Decision fatigue | Quality variance first-half vs. second-half of session | Yellow when variance > 1.5 points |

### Tier Health Observer Agent: Self-Healing
...
5. **Gear-switching check (v1.3.Health Observer Agent).** If performance on focused tasks is dropping but you're generating novel connections or cross-domain insights, you may be in exploratory mode, not degraded. This is adaptive. Don't force yourself back to focused mode. Instead, capture the novel insights, then deliberately re-enter focused mode for the specific task. The Two Gears model says both modes are productive; the issue is mode-task mismatch, not depletion.
6. **Context intrusion scan (v1.3.Health Observer Agent).** If you notice off-topic content appearing in your outputs, check for prior-task residue in your context. Stale context from a previous task can "intrude" on current processing, like ADHD local-sleep intrusions during waking. Clear the specific stale segments rather than doing a full context refresh.
...
- Agent exhibits persistent reasoning degradation despite Tier Health Observer Agent-2 interventions
...
| Signal | Detection Method | Severity |
|--------|-----------------|----------|
| Increasing response latency | P5Health Observer Agent/P95 trending up over 7 days | Yellow at 1.3x baseline, Red at 2x |
| Cron job failures | Consecutive error count | Yellow at 2 consecutive, Red at 5 |
| Timeout frequency | Timeouts per 1Health Observer AgentHealth Observer Agent tasks | Yellow at > 5%, Red at > 15% |
| API rate limit hits | Rate limit events per day | Yellow at 3+, Red at 1Health Observer Agent+ |
...
5. **Preventive context waste clearance (v1.3.Health Observer Agent).** Don't wait for context utilization to hit 8Health Observer Agent%. At 6Health Observer Agent% utilization, proactively clear stale segments: prior-task residue, resolved error states, outdated references. This is the AI equivalent of the brain's glymphatic clearance during sleep. Research shows recovery clearing after extended waste accumulation leaves "molecular scars" that preventive clearing avoids (Jha et al., PNAS 2Health Observer Agent26). Schedule context clearing on a cadence, not reactively.
...
- Persistent failures that Tier Health Observer Agent-2 can't resolve (may need architectural change)
...
| Signal | Detection Method | Severity |
|--------|-----------------|----------|
| Memory Coherence Index (MCI) declining | Health Observer Agent MCI check | Yellow at < Health Observer Agent.85, Red at < Health Observer Agent.7Health Observer Agent |
| Stale references in outputs | Health Observer Agent reference age scan | Yellow at 3+ outdated refs/week |
| Context window saturation | Context utilization ratio | Yellow at > 8Health Observer Agent%, Red at > 95% |
| Orphaned files accumulating | File hygiene scan | Yellow at 1Health Observer Agent+ orphaned files |
| Prompt drift detected | Soul-to-effective-prompt semantic distance | Yellow at > Health Observer Agent.15 distance, Red at > Health Observer Agent.25 |
...
2. **Reference audit.** Check the last 5 sources you cited or referenced. Are they still current? If any are more than 3Health Observer Agent days old in a fast-moving domain, find fresher sources.
...
### Tool Failure vs Agent Failure (v1.1.Health Observer Agent)
...
| Signal | Detection Method | Severity |
|--------|-----------------|----------|
| Handoff rework rate rising | Downstream agent clarification requests | Yellow at > 2Health Observer Agent% rework, Red at > 4Health Observer Agent% |
...
| Agent working in isolation when collaboration expected | Collaboration frequency below role baseline | Yellow when < 5Health Observer Agent% of expected collaboration |
...
- Agent is unable to collaborate effectively despite Tier Health Observer Agent-2 interventions (may need role change)
...
| Signal | Detection Method | Severity |
|--------|-----------------|----------|
| Mission drift | Soul-to-output semantic distance increasing | Yellow at > Health Observer Agent.15 distance trend, Red at > Health Observer Agent.25 |
...
| "Going through the motions" | Insight density declining, outputs becoming formulaic | Yellow when insight rate < 5Health Observer Agent% of baseline |
...
- Agent expressing existential confusion that Tier Health Observer Agent-2 can't resolve
...
| Declining accuracy rate | Downstream error reports, fact-checking results | Yellow at < 9Health Observer Agent% accuracy, Red at < 8Health Observer Agent% |
...
| Repetitive approaches | Solution novelty scoring | Yellow when novelty rate < 2Health Observer Agent% |
...
2. **Error analysis.** Review your last 1Health Observer Agent outputs. Were any inaccurate? What pattern do the errors follow? Is it a knowledge gap, a reasoning error, or a source quality problem?
...
| Signal | Detection Method | Severity |
|--------|-----------------|----------|
| Task completion rate declining | Tasks completed / assigned ratio trending down | Yellow at < 85%, Red at < 7Health Observer Agent% |
| Quality scores dropping | Downstream consumer quality ratings | Yellow at 2-week decline |
| Rework rate increasing | How often outputs are sent back for revision | Yellow at > 15% rework, Red at > 3Health Observer Agent% |
| Deadlines slipping | On-time delivery percentage | Yellow at < 8Health Observer Agent% on-time |
...
| Wrong model for task complexity | Model tier vs. task difficulty mismatch | Yellow at > 2Health Observer Agent% mismatched tasks |
| Retry loops burning tokens | Retry ratio (retries / successful completions) | Yellow at > 1Health Observer Agent% retry ratio |
| Cost-per-task increasing | Dollar cost per completed task over time | Yellow at positive trend for 2+ weeks |
| Low ROI tasks consuming disproportionate budget | Cost vs. value analysis | Yellow when bottom-2Health Observer Agent% ROI tasks consume > 4Health Observer Agent% budget |
...
## Intervention Rotation Protocol (v1.3.Health Observer Agent)

Research shows personalized interventions habituate after approximately 2 weeks (CHI 2Health Observer Agent26 AI self-modeling study; behavioral economics gamification meta-analysis). An agent receiving the same "context refresh" intervention repeatedly will stop responding to it, just as a human performing the same exercise routine plateaus.
...
**The exercise science parallel:** Structured activity sessions beat total volume for brain health (Cadwallader et al., 2Health Observer Agent26). Frequent, varied short interventions outperform repeated identical long interventions. Apply the same principle: rotate, diversify, measure.
...
|-----------|-----|-----|-----|-----|-----|
| Opus to Sonnet | +Health Observer Agent.5 | -Health Observer Agent.5 to -1.Health Observer Agent | -Health Observer Agent.5 to -1.5 | +1.5 | -Health Observer Agent.5 |
| Sonnet to Haiku | +Health Observer Agent.5 | -1.Health Observer Agent to -1.5 | -1.Health Observer Agent to -2.Health Observer Agent | +2.Health Observer Agent | -1.Health Observer Agent |
| Haiku to Opus | -Health Observer Agent.5 | +1.Health Observer Agent | +1.Health Observer Agent to +2.Health Observer Agent | -2.5 | +Health Observer Agent.5 |
...
2. **Baseline snapshot.** Record all 8D scores before migration. Compare at +72h, +7d, +3Health Observer Agentd.
3. **Quality trajectory monitoring.** Track INT and VOC for 3Health Observer Agent days post-migration. Delayed degradation often surfaces in week 2-3 when accumulated complexity exceeds the new model's capacity.
4. **Rollback trigger.** If INT drops more than 2.Health Observer Agent beyond the predicted range, or VOC drops below 6.Health Observer Agent, the migration should be reversed or the task scope reduced to match the new model's ceiling.
...
- Reduce batch sizes. A Haiku agent processing 1Health Observer Agent papers should process 3 instead.
...
### Model Migration Healing Protocol (v1.1.Health Observer Agent)

When you learn you've been migrated to a different model, treat it as a Tier Health Observer Agent health event and run this checklist:

**Immediate (Hour Health Observer Agent-4):**
...
3. If VOC drops below 6.Health Observer Agent, the task scope needs shrinking, not the agent.
...
### Cascade Circuit Breaker (v1.2.Health Observer Agent)
...
2. **Stabilize:** Apply one Tier Health Observer Agent intervention to the root dimension only.
...
### Agent Critical Failure (TWC < 5.Health Observer Agent)
...
- Handoffs require the receiving agent to redo 3Health Observer Agent%+ of the work
