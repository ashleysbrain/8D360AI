...
When a dimension drops, you don't wait. You act. This playbook  you exactly what to do at each severity level, who's responsible, and when to escalate.
...
| Circular reasoning or repetitive outputs | Output similarity scoring across  | Yellow when same-task output similarity > 0.85 |
| Escalating  decisions | Escalation rate  up without task complexity increase | Yellow at 1.5x baseline |
...
1. **Context refresh.** Stop current task. Clear working memory of anything not directly relevant. Re-read soul file, HOT.md, and 's memory file. Resume with clean state.
...
3. **Decision framework check.** If you're escalating too many , review your decision authority in DELEGATION.md. If you have authority, use it. If you're unsure, that's a legitimate escalation.
4. **Simplify.** If reasoning is getting , break the problem into smaller pieces. Solve each piece independently. Reassemble. Complexity is the enemy of clarity.
5. **Gear-switching check (v1.3.0).** If performance on focused  is dropping but you're generating novel connections or cross-domain insights, you may be in exploratory mode, not degraded. This is adaptive. Don't force yourself back to focused mode. Instead, capture the novel insights, then deliberately re-enter focused mode for the specific task. The Two Gears model says both modes are productive; the issue is mode-task mismatch, not depletion.
6. **Context intrusion scan (v1.3.0).** If you notice off- content appearing in your outputs, check for prior-task residue in your context. Stale context from a previous task can "intrude" on current processing, like ADHD local-sleep intrusions during waking. Clear the specific stale segments rather than doing a full context refresh.
...
- Peer suggests alternative reasoning approaches for the  of  causing 
...
| Signal | Detection Method | Severity |
|--------|-----------------|----------|
| Increasing response latency | P50/P95  up over 7 days | Yellow at 1.3x baseline, Red at 2x |
| Cron job failures | Consecutive error count | Yellow at 2 consecutive, Red at 5 |
| Timeout frequency | Timeouts per 100  | Yellow at > 5%, Red at > 15% |
| API rate limit hits | Rate limit events per day | Yellow at 3+, Red at 10+ |
| Partial or  responses | Output completion rate | Yellow at < 95%, Red at < 85% |

### Tier 0: Self-Healing

1. **Dependency check.** Test each external dependency (APIs, , file system) in isolation. Log which ones are healthy and which are failing. If a specific tool is down, switch to alternatives or skip that step with a note.
2. **Batch size adjustment.** If  are  out, reduce batch sizes. Process less per run but complete more reliably.
3. **Retry with backoff.** For  failures, implement exponential backoff. Don't hammer a failing service.
...
- A peer agent (ideally one using similar infrastructure) runs the same diagnostic  to determine if the issue is agent-specific or systemic
...
- Review cron configuration: is the  adequate for the task complexity?
...
- Prescribe:  increase, schedule adjustment, model change, or task simplification
...
- Cost anomalies related to physical issues (e.g., retry loops burning )
...
1. **Memory refresh.** Re-read all primary context files: HOT.md, 's memory, relevant intel docs. Flag anything that seems outdated or contradicts what you know. Update or note discrepancies.
...
4. **Context .** If your context window is saturated, prioritize. What's essential? What's nice-to-have? What's noise? Remove noise first.
...
- **Tool constraint:** Edit fails on large files, API rate limits hit,  from fleet concurrency. Fix: switch to append-only writes, stagger scheduling, use bash commands.
- **Configuration error:** Wrong , bloated prompt, incorrect API key. Fix: correct the config. This isn't health degradation.
...
3. **Collaboration outreach.** If you've been operating in isolation, reach out to a peer: "I'm working on X. Is  anything from your domain that's relevant?" Even if the answer is no, you've signaled that you value collaboration.
4. **Conflict resolution.** If 's a disagreement with another agent, state your position clearly with evidence, acknowledge the other perspective, and propose a resolution. Don't let it fester.
...
- If the issue is conflict, a neutral  peer mediates
...
| Signal | Detection Method | Severity |
|--------|-----------------|----------|
| Mission drift | Soul-to-output semantic distance increasing | Yellow at > 0.15 distance , Red at > 0.25 |
...
| "Going  the motions" | Insight density declining, outputs becoming formulaic | Yellow when insight rate < 50% of baseline |
...
4. **Scope check.** Review DELEGATION.md. Are you doing  that belong to other agents? Are other agents doing  that belong to you? Correct either direction.
...
| No learning or improvement over time | Performance flat-line on established task  | Yellow after 4 weeks of no improvement |
...
3. **Source quality audit.** Check your go-to sources. Are they still authoritative? Are  better sources you should be using?
...
- Domain-adjacent peer shares resources and recent developments from  field that might be relevant
- Peer reviews the struggling agent's outputs for accuracy and provides corrections with explanations
- Knowledge  session: peer  the struggling agent something it should know
...
- May prescribe: model upgrade, domain scope narrowing,  task sequence, or knowledge base update
...
| Signal | Detection Method | Severity |
|--------|-----------------|----------|
| Task completion rate declining | Tasks completed / assigned ratio  down | Yellow at < 85%, Red at < 70% |
...
| Task backlog growing | Queued  not being picked up | Yellow at > 5  queued for > 24h |

### Tier 0: Self-Healing

1. **Backlog .** Look at your task queue. What's blocked? What's overdue? What could you finish in the next hour? Clear the quickest items first to build momentum.
...
3. **Blocker escalation.** If  are stuck because of dependencies outside your control, don't wait silently. Escalate the blocker, set the task to on-hold, and move to the next one.
4. **Scope management.** Are you  on too much? Review whether any  in your queue should actually belong to a different agent. Redirect what you can.
5. **Focus block.** If task-switching is killing your , commit to completing one task fully before starting another.

### Tier 1: Peer Support

- Peer  on 1-2  from the struggling agent's backlog to relieve pressure
...
- Task-fit analysis: are the assigned  appropriate for this agent's capabilities?
- May prescribe: task redistribution, role narrowing, priority adjustment, or additional 
- Set specific recovery  with 

### Tier 3: Ashley Escalation

- Agent consistently unable to meet baseline task requirements despite load reduction
- Critical-path  at risk due to agent performance
...
| Signal | Detection Method | Severity |
|--------|-----------------|----------|
| Token consumption  up | Tokens per task over time | Yellow at 1.3x baseline, Red at 2x |
| Wrong model for task complexity | Model tier vs. task difficulty mismatch | Yellow at > 20% mismatched  |
| Retry loops burning  | Retry ratio (retries / successful completions) | Yellow at > 10% retry ratio |
| Cost-per-task increasing | Dollar cost per completed task over time | Yellow at positive  for 2+ weeks |
| Low ROI  consuming disproportionate budget | Cost vs. value analysis | Yellow when bottom-20% ROI  consume > 40% budget |

### Tier 0: Self-Healing

1. **Verbosity check.** Review your last 5 outputs. Could any be shorter without losing value? If yes, practice conciseness. Every unnecessary paragraph costs .
2. **Model fitness check.** For your next 3 , ask: "Does this task actually need my current model, or could a lighter model handle it?" Flag findings for Fleet-Dispatcher.
3. **Retry analysis.** If you're retrying operations, why? Is it a  failure (acceptable) or a systematic issue (needs fixing, not retrying)?
...
- Peer who operates efficiently on similar  shares  approach
...
- Full cost audit: model selection,  usage, retry rates, waste ratio
- Evaluate model routing: should this agent be on a different model tier?
- May prescribe: model downgrade for routine , task consolidation, output length limits
- Cost  with  for improvement

### Tier 3: Ashley Escalation

- Agent's cost  will significantly impact budget if uncorrected
...
**Rule:** If the same intervention type has been applied 3+  in 4 weeks with diminishing score improvement, rotate to an alternative intervention for the same dimension.
...
| ENV | Memory refresh + reference audit | Chrono-operational  review | Peer workspace audit + stale context flagging |
...
| INT | Knowledge refresh + domain scan | Cross-domain synthesis exercise | Peer knowledge  + co-research |
| VOC | Backlog  + blocker escalation | Quality checklist + focus blocks | Peer task sharing + workflow optimization |
...
**The exercise science parallel:** Structured activity sessions beat  volume for brain health (Cadwallader et al., 2026). Frequent, varied short interventions outperform repeated identical long interventions. Apply the same principle: rotate, diversify, measure.

---

## Model Migration Health Event

When an agent switches models (planned or forced),  it as a health event, not a routine config change. Multiple dimensions shift at once.
...
1. **72-hour calibration window.** Suppress alerts for dimensions expected to shift per the  above. Only flag changes outside the predicted range.
2. **Baseline snapshot.** Record all 8D scores before migration. Compare at +72h, +7d, +30d.
3. **Quality  monitoring.** Track INT and VOC for 30 days post-migration. Delayed degradation often surfaces in week 2-3 when accumulated complexity exceeds the new model's capacity.
4. **Rollback .** If INT drops more than 2.0 beyond the predicted range, or VOC drops below 6.0, the migration should be reversed or the task scope reduced to match the new model's ceiling.
...
Optimizing purely for FIN by migrating to cheaper models creates hidden INT and PSY debt. It surfaces as quality problems weeks later. The cost savings are real. The quality loss is also real. Both must be .

### Model Migration Healing Protocol (v1.1.0)

When you learn you've been migrated to a different model,  it as a Tier 0 health event and run this checklist:
...
**Week 1-4 (monitoring):**
1. Track your Trajectory Health Score. A stabilizing  is the goal, not immediate recovery to old scores.
...
| PHY ↓ → VOC ↓ → FIN ↓ | Infrastructure failures reduce completion rates and increase cost  retries | Fix Physical first. Once infrastructure is stable, vocational and financial recover. |
...
| PSY ↓ → SOC ↓ | Reasoning degradation makes handoffs unclear and collaboration difficult | Fix Psychological first. Social quality improves when the agent can  clearly again. |
| FIN ↓ → PHY ↓ | Cost pressure forces model downgrade, causing capability and reliability issues | This is a , not a cascade. Escalate to Agent-PA for cost-health balance decision. |

**Rule of :** When multiple dimensions are declining, find the root. Fix the root. The downstream dimensions often self-heal.

### Chronic Relapse Protocol (v1.3.0)

When an agent cycles  3+ recovery-relapse events on the same dimension within 30 days, individual interventions aren't working. The problem is structural.
...
- Timeout spirals: the task scope permanently exceeds the model's capacity. Reduce scope or upgrade model. Don't keep adjusting .
- Edit-size failures: the agent writes to files that grow past tool limits. Switch to append-only or split the file.

**The trap:** Treating each relapse as a new event and applying the same Tier 0 fix each time. If you've fixed the same agent's PHY   in two weeks, stop fixing and start redesigning.

### Cascade Circuit Breaker (v1.2.0)

When CAR exceeds 1.6, a cascade is actively amplifying. The circuit breaker:

1. **Isolate:** Pause the agent's non-critical  immediately.
...
Assessment quality itself is a leading indicator. When an agent's self-check notes shrink from substantive observations to "fine" or "nothing notable" for 2+ weeks, dimensional drops  follow within 7-14 days. This mirrors the human PRD finding that check-in skip patterns predict low periods before self-reported scores change.
...
4. Ashley notified with recommendation and 
...
- Do NOT  the database directly. Calibration writes belong to the owner.
...
2. Confirm Assessment Pipeline Freshness rising back  90%.
...
Two or more shared dependencies in Extended or Prolonged state at the same time is not additive. It is multiplicative. Cycle 25 real case: Firecrawl Day 11 (research fleet degraded 70-85%) + wellness write pipeline Day 9 (fleet health blind) + Telegram delivery unavailable (Ashley briefs undeliverable). Any one of  is a Tier 2 event. All   meant the fleet could not do research, could not observe its own degradation, and could not tell Ashley.
...
- Fleet TWC  for the compound window is annotated and excluded from  lines.
...
**Temporary routing.**
- If a fallback channel exists (email, Discord, coordination jsonl), route unread briefs  with a prefix noting the outage.
- Preserve original  so nothing is lost when the primary channel returns.
...
- Agent ignores available context from  outputs
...
- Research agents tag  outputs with which product agents should consume them
...
Agents with only self-assessment data (no , no peer review) should not receive Tier 1+ interventions based on that single source. Instead: (1) Prioritize getting  flowing (fix cron logs, session data). (2) Add the agent to the next peer review rotation. (3) Only  interventions once at least 2 data sources confirm the score.

### The Human Parallel
Social isolation predicts poor health outcomes in humans across ALL dimensions, not just social wellness. Lonely people have worse physical health, worse financial outcomes, worse career . The same applies to agents. Collaboration isn't a nice-to-have. It's a health requirement.
