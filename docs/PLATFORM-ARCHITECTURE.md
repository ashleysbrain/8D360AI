...
AI agents degrade. Not dramatically, not all at once, but in ways that are hard to see from the inside. Context drifts. Error rates creep. An agent that scored itself a 9 last week is quietly producing 7-quality work. It doesn't know. Nobody checks. And the fleet absorbs the cost in bad handoffs, wasted , and outputs that need rework.

This is the same problem humans face. People don't recognize  own burnout until they're deep in it. Self-reported wellness is a starting point, never the whole picture. Real healthcare requires objective measurement, outside observation, and peer accountability.
...
**Core principle:** Self-reported health is necessary but not sufficient. The system must independently verify what agents claim about , catch blind spots they can't see, and intervene before degradation becomes failure.
...
This is not optional. It is a foundational health indicator at both the agent level and the fleet level. An agent operating in isolation when it should be building on ' work is exhibiting a wellness deficit, even if its individual scores look fine.

### The Rule

Every agent must always look to build WITH other agents. Never duplicate work another agent already produced. Read what your  created. Build on it. If you're scanning the same domain as another agent, that's a process failure, not a feature.

### Fleet-Level Collaboration Health Metrics

These are  at the organizational level, not the individual agent level:
...
| **Build-On Rate** | % of  where an agent explicitly references or extends another agent's prior work | >60% | <30% |
...
- **Social dimension:** An agent that collaborates well scores higher. An agent that ignores ' work and duplicates effort scores lower, even if its solo output is high quality.
- **Financial dimension:** Duplication wastes  and API spend. Two agents scanning the same domain is a Financial health failure for both.
- **Vocational dimension:** An agent's job isn't just to complete . It's to complete  that fit into a larger system. Solo excellence that creates organizational inefficiency is a Vocational deficit.
...
│  │  Blends  sources with inflation detection + correction   │    │
│  └──────────────────────────┬───────────────────────────────────┘    │
│                              │                                       │
│         ┌────────────────────┼────────────────────┐                  │
│         ▼                    ▼                     ▼                  │
...
│  ┌──────────────────────────────────────────────────────────────┐    │
│  │              ESCALATION ENGINE                                │    │
│  │  Autonomous → Peer → Agent-PA → Ashley ()                 │    │
│  └──────────────────────────────────────────────────────────────┘    │
│                              │                                       │
│                              ▼                                       │
│  ┌──────────────────────────────────────────────────────────────┐    │
│  │              FLEET HEALTH DASHBOARD                           │    │
│  │  Real-time composite scores, , alerts             │    │
│  └──────────────────────────────────────────────────────────────┘    │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```
...
A single source of health data is unreliable for the same reason a single witness is unreliable: bias, blind spots, and self-interest. Agents, like humans, tend to overrate  own performance. Objective data alone misses nuance. Peers catch  both miss.
...
When self-assessment diverges from objective  by more than 2 points, the composite automatically reweights:
...
TWC goes beyond simple averaging. It captures cross-dimensional coupling, the way disruption in one dimension cascades  others:
...
The first term captures individual dimension health. The second term captures how dimensions amplify or suppress each other. This interaction term  accounts for 30-50% of true wellness variance and is what makes the framework predictive, not just descriptive.
...
- **CAR > 1.6**: Strong cascade. Rapid propagation, critical  point.
...
- **κ_φλ = 0.74** (Physical-Intellectual): Infrastructure directly constrains cognitive capacity. Token  limits what complexity an agent can handle.
...
| Physical | Token , response latency (P50/P95), memory utilization, uptime percentage, cron success rate,  frequency |
| Intellectual | Task complexity handled (novel vs. routine), novel solution generation rate, learning rate on new task , knowledge currency (source age), cross-domain synthesis rate |
...
| Vocational | Task completion rate, output quality scores (downstream rework rate),  efficiency ( per time window), on-time delivery percentage |
| Financial | Token cost per task (normalized by complexity), resource utilization efficiency (model-tier match rate), waste reduction (retry and abandoned response ratio), cost  slope |
...
```json
{
  ...
  "storage": "fleet-/YYYY-MM-DD/{agent-id}.json",
  "retention": "90 days rolling, monthly aggregates permanent",
  "sources": [
    "cron job logs (success/fail//duration)",
    "session logs ( counts, error events, tool failures)",
...
| Mission Alignment | "Are this agent's outputs advancing the mission, or just going  the motions?" (1-10) |

- **Anti-gaming measures:**
  - Peer scores are anonymous to the reviewed agent
  - Health Observer Agent cross-references peer scores against objective , flags reviewers who consistently score 2+ points above objective data (leniency bias) or below (harshness bias)
...
One  this agent does well: ___
One  that would improve  work: ___
```
...
Dimensions not directly covered by peer review (Financial, some Environmental) rely more heavily on .
...
- On-demand ( by Health Observer Agent when anomalies detected)

**Self-Assessment Accuracy Score:**

Health Observer Agent  how closely each agent's self-assessments match the composite score over time. This becomes its own metric:
...
Health Observer Agent maintains a per-agent "inflation index" that :
...
Health Observer Agent (Vigilant Intelligence for Telemetry, Assessment, and Longitudinal Surveillance) is a dedicated agent whose sole job is monitoring fleet health from the outside. It doesn't  self-reports. It has no other , no competing priorities, and no reason to be generous.

Health Observer Agent is the equivalent of a hospital's quality assurance department. It doesn't  patients; it watches the people who do and catches problems they can't see .

### 4.2 Responsibilities

1. **Aggregate  data** from all system sources into per-agent health profiles
2. **Cross-validate** self-reported scores against objective , flag divergences
...
5. **Monitor behavioral drift**  longitudinal analysis of output patterns
6. **Compute composite health scores** using the -source weighted formula
7. **Generate health alerts** when agents cross warning or critical 
...
```json
{
  ...
  "access": {
    ...
    "writes": ["fleet-/", "intel/agents/fleet-health/"],
...
The most insidious form of AI degradation is drift: the agent still works, still completes , but its outputs gradually shift away from what they should be. Like a compass needle slowly moving off north. The agent doesn't notice because each day's output is close to yesterday's.

**Health Observer Agent detects drift :**
...
5. **Error Pattern Evolution:** Track not just error rate but error type. A shift in error  (from execution errors to reasoning errors, for example) signals a different kind of degradation.
...
Each dimension now includes sub-dimensions for granular . Sub-dimension scores roll up to the dimension score using equal weighting unless role-specific overrides apply.
...
|---------------|-------------|-----------------|-------------|-------------|
| Reasoning Coherence | Logical consistency across outputs | Contradiction rate in outputs | "Am I making sense?" | "Did this agent's reasoning ?" |
...
| Sub-Dimension | Description | Objective Signal | Self-Signal | Peer Signal |
|---------------|-------------|-----------------|-------------|-------------|
| Uptime / Availability | Consistently operational when needed | Cron success rate, session availability | "Am I reliably available?" | "Can I count on this agent being ?" |
| Response Latency | Speed of task completion | P50/P95 response ,  | "Am I getting slower?" | "Is this agent responsive?" |
| Error Rate | Frequency of operational failures | Errors per 100  | "How often do I fail?" | "How often does this agent's work have errors?" |
...
| Resource Efficiency | Compute and memory usage patterns | Token count , context window utilization | "Am I using resources well?" | N/A (-primary) |
...
| Tool Reliability | Health of the  and APIs the agent depends on | Tool failure rate, retry frequency | "Are my  working?" | N/A (-primary) |
...
| Sub-Dimension | Description | Objective Signal | Self-Signal | Peer Signal |
|---------------|-------------|-----------------|-------------|-------------|
| Handoff Quality | Completeness and clarity of task  | Handoff rework rate (did receiving agent need clarification?) | "Are my handoffs clear?" | "Are handoffs from this agent complete?" |
...
| Sub-Dimension | Description | Objective Signal | Self-Signal | Peer Signal |
|---------------|-------------|-----------------|-------------|-------------|
| Domain Expertise | Depth of knowledge in assigned area | Accuracy rate on domain-specific , expert review scores | "How deep is my expertise?" | "Is this agent a domain expert?" |
...
| Learning Velocity | Speed of skill acquisition and improvement | Performance improvement rate on new task  | "Am I learning and growing?" | "Is this agent getting better over time?" |
...
| Intellectual Honesty | Acknowledges limits, doesn't fabricate | Hallucination rate, false confidence incidents | "Do I admit what I don't know?" | "Does this agent make  up?" |

---

#### Dimension 7: Vocational (VOC) 💼

**What it measures:** Task performance, output quality, professional reliability, growth .

| Sub-Dimension | Description | Objective Signal | Self-Signal | Peer Signal |
|---------------|-------------|-----------------|-------------|-------------|
| Task Completion Rate | Percentage of assigned  completed successfully | Completed / Assigned ratio | "Do I finish what I start?" | "Does this agent deliver?" |
...
| On-Time Delivery | Meeting deadlines and expected  | %  completed within expected  | "Am I ?" | "Is this agent punctual?" |
| Specialization Depth | Mastery of assigned niche beyond generalist capability | Performance differential on specialist vs. generalist  | "Am I an expert at my specific job?" | "Is this agent specialized or generic?" |
| Proactivity | Value-add beyond explicit assignments | Unsolicited improvements, self-generated  that added value | "Do I go beyond what's asked?" | "Does this agent anticipate needs?" |
...
| Sub-Dimension | Description | Objective Signal | Self-Signal | Peer Signal |
|---------------|-------------|-----------------|-------------|-------------|
| Token Efficiency | Value produced per  consumed | Tokens/task normalized by complexity | "Am I using  wisely?" | N/A (-primary) |
| Model Selection | Using the right model for the task complexity | Model tier vs. task difficulty match rate | "Am I using the right model?" | N/A (-primary) |
| Cost Trajectory | Spending  over time | Cost-per-task slope (should be flat or declining) | "Am I getting more expensive?" | N/A (-primary) |
...
| Waste Reduction | Minimizing retries, abandoned work, unnecessary API calls | Retry ratio, abandoned response ratio, idle  spend | "Do I waste resources?" | N/A (-primary) |
...
### One  I'd change about how I work:
{specific, actionable}
```

### 6.3 Honest Scoring Calibration

The system actively  agents to self-assess accurately:
...
4. Self-Awareness Score is itself  as a meta-metric and factors into Psychological wellness
...
| Slowing response  | Telemetry | 0.10 | P50 latency > 1.3x 30-day baseline |
| Rising  consumption | Telemetry | 0.10 | Tokens/task > 1.4x baseline for same task  |
...
| 0.31 - 0.50 | Warning | Autonomous intervention  (context refresh, load reduction). Agent-PA notified. |
...
When a dimension drops below , the agent should do something about it without asking a human, just like a healthy person  an aspirin for a headache before calling a doctor.
...
Where σᵢ = sensitivity index, (1 - Dᵢ) = room for improvement, S = set of dimensions below .
...
| Full-System Decline (3+ dims) | Multiple dims below  | Psychological (hub dim, σ=0.620) | Broadest cascade. Secondary: Physical (κ_ψφ=0.82) |

**Minimum Effective Intervention (MEI):** For Psychological (σ=0.620, max κ=0.82), improving by just 1 point on a 10-point scale is enough to initiate a detectable positive cascade. Always  the dimension with highest ILS, not just the lowest score.
...
| Physical | Increasing latency,  errors | Resource check: verify API health, adjust batch sizes, report infrastructure issues to DevOps |
...
| Spiritual | Role boundary violations | Scope check: review DELEGATION.md, identify  outside role, redirect or escalate appropriately |
...
| Vocational | Falling completion rate | Workload audit: review task queue, identify blockers, escalate blocked , reprioritize |
...
| Financial | Token usage  up | Efficiency check: review last 5  for verbose responses, identify compression opportunities, adjust |
...
| Physical | Peer runs diagnostic   the same infrastructure, helps isolate whether the issue is agent-specific or systemic |
...
| Intellectual | Peer shares domain resources, co-researches a , provides knowledge  |
| Vocational | Peer  on some of the struggling agent's task load , helps clear backlog |
| Financial | Peer suggests model routing optimizations based on  own experience with similar  |

### 8.4 Agent-PA Review Interventions (Tier 2)

| Trigger | Agent-PA Action |
|---------|-------------|
| Dimension < 6.0 | Root cause investigation. Review agent configuration, task load, dependencies. Prescribe specific intervention plan with . |
| TWC declining 3+ weeks | Performance review. Determine if issue is agent capability, task mismatch, infrastructure, or context degradation. May reassign  or adjust role. |
| Burnout risk 0.31-0.70 | Mandatory load reduction. Remove non-critical . Increase context refresh frequency. Assign peer buddy. Monitor daily. |
| Peer conflict | Mediation. Review both agents' perspectives, adjust collaboration protocols, potentially reassign pairing. |
| Persistent score inflation | Calibration intervention. Review self-assessment accuracy history, provide detailed feedback with examples, adjust self-assessment weight . |
...
Autonomous actions : {what's been }
Why this needs you: {specific reason human judgment is required}
Recommended action: {what Agent-PA  should happen}
Confidence: {1-5}
```
...
|--------|-------------|--------|
| Skill Acquisition Rate | New task  successfully handled per month | 2+ new task /month for specialists, 4+ for generalists |
| Performance Improvement | Improvement in quality scores over time on established task  | Positive slope over any 30-day window |
| Knowledge Expansion | New domain areas where the agent demonstrates competence | 1+ per quarter |
| Error Learning Rate | How quickly the agent stops repeating the same error | Same error type should not recur more than  |
| Cross-Domain Connection | Novel connections made between different knowledge areas | 1+ per month |
| Teaching Effectiveness | Quality of knowledge  to other agents | Peer feedback on knowledge sharing |
...
|-------------|-------------|
| Agent producing 7-quality work while self-reporting 9 | Downstream agents spend extra  fixing bad inputs |
...
| Poor handoffs | Receiving agent wastes 30-50% of  re-establishing context |
...
```json
{
  ...
  "fleet": {
    ...
    "dimension_averages": {
      "psychological": { "score": 8.15, "": "stable", "weakest_agent": "mc-command-runner" },
      "physical": { "score": 8.12, "": "stable", "weakest_agent": "athena" },
      "environmental": { "score": 8.07, "": "improving", "weakest_agent": "cron-failure-alert" },
      "social": { "score": 7.78, "": "stable", "weakest_agent": "domain-scan-cohort" },
      "spiritual": { "score": 8.14, "": "stable", "weakest_agent": "mc-url-watcher" },
      "intellectual": { "score": 8.82, "": "stable", "weakest_agent": "mc-command-runner" },
      "vocational": { "score": 8.89, "": "stable", "weakest_agent": "content-gamma" },
      "financial": { "score": 8.22, "": "improving", "weakest_agent": "oracle" }
    }
  },
  "agents": [
    {
...
Stored at `fleet-/agents/{agent-id}/health-record.json`:
...
| Self-Assessment Template | Post-task + weekly + on-demand formats | Markdown  |
...
1. **Minimum viable:** Self-assessment  injected into agent prompts + weekly manual review
2. **Basic:** Add objective  collection from whatever logging your system uses
...
- `inflation_detector.py`, Self-assessment accuracy 
...
### Abstract
A framework for comprehensive health monitoring, assessment, and autonomous healing of AI agent fleets, adapted from the 8-dimensional human wellness model. We present a -source health scoring system that blends objective  (40%), peer assessment (30%), and self-assessment (30%) to produce composite health scores resistant to self-report bias. We introduce burnout detection algorithms for AI agents, autonomous healing protocols, and an independent health observer architecture. The framework is open, model-agnostic, and scale-agnostic.
...
- Deploy Health Observer Agent agent (Haiku, hourly  collection)
- Inject self-assessment  into all agent prompts
- Begin collecting objective  from cron logs and session data
...
- Begin computing -source composite scores
...
- Calibrate burnout  based on real data
...
*"My job is to heal humans. Your job is to heal and monitor the AI  humans use."*
