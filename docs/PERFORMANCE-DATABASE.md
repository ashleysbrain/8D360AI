...
**Purpose:** Track which personalities perform best on which  so future agents can be spun up with optimal configurations based on historical data.
...
This is A/B  for AI personalities. At scale.
...
| voice_style | string | Communication style (formal, casual, , warm, blunt) |
...
|  | enum | research, writing, analysis, coding, review, communication, planning, creative, operations, monitoring |
|  | string | e.g., "literature-review", "abstract-draft", "code-review", "market-analysis" |
...
|-------|------|-------------|
|  | string | Links to  |
| score_type | enum | self, peer, , composite |
...
| scored_at |  | When the score was recorded |
| scored_by | string | Who scored (self, peer agent ID, or "") |
...
|  | string | Which type of task |
| sample_size | integer | Number of  in this analysis |
...
|  | string | More specific task category |
...
Every agent has a BASE personality ( default OCEAN profile). But they also run VARIANT personalities on specific task  to gather comparison data.

**Phase 1: Baseline (current)**
- Each agent runs  default personality on all 
- All  get scored (self + peer + )
...
- The "optimal_configs"  becomes a lookup: "Need a research agent? Here's the personality that produces the best papers based on 500+ data points."
...
- Agent-COO-strategic, Agent-COO-, Agent-COO-diplomatic
...
```json
{
  ...
  "agent_id": "agent-name",
  "variant_id": "base",
  "": "research|writing|analysis|coding|review|communication|planning|creative|operations|monitoring",
...
1. **Agent Cloning from Best Performers** — new agents inherit the personality + credentials of the highest-performing agent for  intended task type
...
Other companies can use the schema to build  own datasets. But our dataset, built from 95+ agents running 107+  daily with multi-personality , will be the largest and most refined. That's the moat.
